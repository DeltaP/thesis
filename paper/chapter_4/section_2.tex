...
Discrete scaling symmetry, shown in \ref{fig:blocking} simply means that you can divide the sites of the lattice into regular blocks that tile the entire lattice and then replace those blocks with a single site.
...
If we perform this replacement the same way to every block in the original lattice and the lattice formed by the block variables is identical after scaling the lattice spacing $a\rightarrow a'\equiv sa$ to the original lattice then the system has discrete scaling symmetry.
...
An $14^2$ lattice can only be blocked once, $14^2 \rightarrow 7^2$, $7^2$ can not be blocked with a scale factor of 2.
...
This allows us to efficiently store a lattice blocked down to any allowable level and has the added benefit that only minor adjustments need to be made to calculate blocked observables.
...
\begin{equation}
  ...
  \xi^{n_b}=s^{-n}\xi^{(0)}.
\end{equation}
Critical fixed points exist when $\xi = \inf$ and trivial fixed points exist when $\xi = 0$.
...
Figure \ref{fig:Kflow} visualizes the renormalization group flow that I have described for a system with one fixed point and only one relevant coupling at the fixed point, $K_0$.
...
Figure \ref{fig:Kflow2lm} demonstrates how two lattice matching is performed.
...
RG flows will flow to the renormalized trajectory and then along it as shown in \ref{fig:}

\subsection{Confining Theories}
...
The behavior of a conformal theory is shown in figure \ref{fig:}.
