...
This effectively integrates out the ultraviolet degrees of freedom.
As we integrate out the UV we move towards the renormalized trajectory.
In our blocking step we also apply a nHYP smearing step.
The smearing step chooses the renormalized trajectory that we will flow to under sucessive blockings.
If we start with an infitinte volume it would the choice of nHYP parameters would be of little consequence because we could block our system an infitinte number of times until we reach the renormalized trajectory.
Unfortunatley in the real world we are not afforded that luxury.
It is necessary to choose a smearing that gaurantees you reach the renormalized trajectory, more on this in the next section.
...
We proceed by repeatedly applying RG blocking transformations (with scale factor $s = 2$) to lattices of volume $24^3\X48$, $12^3\X24$ and $6^3\X12$.
...
In the following subsection, I describe how we perform this optimization.
